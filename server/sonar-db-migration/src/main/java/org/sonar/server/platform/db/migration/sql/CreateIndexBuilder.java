...
public class CreateIndexBuilder {
    ...
    /**
     * Required name of index. Name must be unique among all the tables of the
     * schema.
     */
    ...
    /**
     * Add a column to the scope of index. Order of calls to this method is
     * important and is kept as-is when creating the index. The attributes used
     * from {@link ColumnDef} are the name, the type and the length (in case of
     * VARCHAR). Other attributes are ignored.
     */
    ...
    private String columnSql(ColumnDef column) {
        ...
        if ((dialect.getId().equals(MySql.ID) || dialect.getId().equals(MariaDb.ID)) && column instanceof VarcharColumnDef) {
